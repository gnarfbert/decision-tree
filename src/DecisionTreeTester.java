...
public class DecisionTreeTester<G extends ITreeGenerator<D>, D extends IDataset> {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String trainingPath = DATA_BASE + userInput.nextLine();
            System.out.println("Enter the file name of the testing file: ");
            String testingPath = DATA_BASE + userInput.nextLine();
            System.out.println("Enter the categorical attribute of interest: ");
            String targetAttribute = userInput.nextLine();

            Dataset trainingData = makeDataset(trainingPath, Dataset.class);
            double accuracy =
                    tester.getDecisionTreeAccuracy(trainingData, trainingData, targetAttribute);
            System.out.println("Accuracy on training data: " + accuracy);

            int numIters = 100;
            Dataset testingData = makeDataset(testingPath, Dataset.class);
            accuracy = tester.getAverageDecisionTreeAccuracy(trainingData, testingData, targetAttribute, numIters);
